...
This repository contains the code used to analyze the storm-induced orbital decay of eleven LEO satellites during May 1–14, 2024, in relation to space weather. Space weather and orbit data were processed via a Snakemake workflow. Data and results are archived on Zenodo and can be used to reproduce our findings by running the two notebooks in `./notebooks`.
...
### Project Organization
    ...
    ├── Zenodo/                                    <- Main data and results directory.
        ...
        └── results/                               <- [needed] Different results generated from the workflow, from figures, to 
            └── 2024-05-01_2024-05-14/             <-
                ├── figures/                       <- [output of ./notebooks] All figures generated for the manuscript, produced by notebooks in ./notebooks 
                ├── disturbance_annotations/       <- [output of ./notebooks ] All figures with annotations of in-orbit disturbance strengths
                ├── cors/                          <- [output of ./notebooks ] Plots of lags vs. correlation and estimated response times by region )
                ├── satellites/<satellite>/
                                       ...
                                       └── plots   <- [output of ./notebooks ] Prediction plots per satellite, as in the manuscript
    ...
    └── leo_orbital_decay_rate_impacts/src/        <- Source code directory 
...
<p><small>Folders marked with `[needed]` are used and must be populated with the corresponding data to run the notebooks and reproduce the results. Folders marked with `[output of ./notebooks ]` are generated by running the two notebooks, and contain all figures and results used in the manuscript.</small></p>
...
[![DOI](https://img.shields.io/badge/DOI-10.5281/zenodo.19115188-blue)](https://doi.org/10.5281/zenodo.19115188)
